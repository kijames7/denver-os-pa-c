...
//Used define in place of static const
...
pool_pt mem_pool_open(size_t size, alloc_policy policy) {
    ...
    pool_manager = malloc(sizeof(pool_mgr_t));
    ...
    pool_manager->node_heap = malloc(MEM_NODE_HEAP_INIT_CAPACITY * sizeof(node_t));
...
alloc_status mem_pool_close(pool_pt pool) {
    ...
    // No decrementing the pool_store_size since it will only grow
    // free mgr
    free(pool_mgr);
...
void mem_inspect_pool(pool_pt pool, pool_segment_pt *segments, unsigned *num_segments) {
    ...
    segs = malloc(pool_mgr->used_nodes * sizeof(pool_segment_t));
    if(segments == NULL){
        return;
    }
    // node heap and segment array loop
    // write the size and allocation of each segment for every node then return the values
...
static alloc_status _mem_resize_pool_store() {
    ...
    if (((float) pool_store_size / pool_store_capacity) > MEM_POOL_STORE_FILL_FACTOR) {
        if((pool_store = realloc(pool_store, pool_store_capacity * MEM_POOL_STORE_EXPAND_FACTOR * sizeof(pool_mgr_pt)))){
...
static alloc_status _mem_resize_node_heap(pool_mgr_pt pool_mgr) {
    ...
    if (((float) pool_mgr->node_heap->used / pool_mgr->node_heap->alloc_record.size) > MEM_NODE_HEAP_FILL_FACTOR){
        if((pool_mgr->node_heap = realloc(pool_mgr->node_heap, MEM_NODE_HEAP_EXPAND_FACTOR * MEM_NODE_HEAP_INIT_CAPACITY * sizeof(node_t)))){ pool_mgr->node_heap->alloc_record.size *= MEM_NODE_HEAP_EXPAND_FACTOR;
            return ALLOC_OK;
        }
    }
    return ALLOC_FAIL;
...
static alloc_status _mem_resize_gap_ix(pool_mgr_pt pool_mgr) {
    ...
    if(((float)pool_mgr->pool.num_gaps / pool_mgr->gap_ix_capacity) > MEM_GAP_IX_FILL_FACTOR){
        ...
        if((pool_mgr->gap_ix = realloc(pool_mgr->gap_ix, MEM_GAP_IX_EXPAND_FACTOR * MEM_GAP_IX_INIT_CAPACITY * sizeof(gap_t)))){
...
static alloc_status _mem_remove_from_gap_ix(pool_mgr_pt pool_mgr, size_t size, node_pt node) {
    ...
    for(int i = pos; i < pool_mgr->pool.num_gaps - 1; i++){
